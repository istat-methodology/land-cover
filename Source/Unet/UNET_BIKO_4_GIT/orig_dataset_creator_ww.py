...
def dataset_creator(base_path):
    ...
    #train
    x_train_name = natsorted([f for f in glob(base_path+'TRAIN/River/'+'*.tif')])
    y_train_name = natsorted([f for f in glob(base_path+'TRAIN/River//'+'*.jpg') if 'Mask' in f])

    #test
    x_val_name = natsorted([f for f in glob(base_path+'TEST/River/'+'*.tif')])
    y_val_name = natsorted([f for f in glob(base_path+'TEST/River/'+'*.jpg' ) if 'Mask' in f])

    #rgb val
    x_val_RGB_file  = natsorted([f for f in tqdm(glob(base_path+'TEST/River/'+'*.jpg')) if not 'Mask' in f])
    x_train_RGB_file  = natsorted([f for f in tqdm(glob(base_path+'TRAIN/River/'+'*.jpg')) if not 'Mask' in f])
    ...
    np.save(save_path+'x_train_river.npy',x_train_final)
    np.save(save_path+'y_train_river.npy',y_train_final)
    np.save(save_path+'x_val_river.npy',x_val_final)
    np.save(save_path+'y_val_river.npy',y_val_final)


    np.save(save_path+'x_val_RGB_river.npy',x_val_final_RGB)
