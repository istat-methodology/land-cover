...
#ImageInput='/IstatDL/Users/public/Datasets/PISA.tif'
ImageInput='/mnt/Datasets/Toscana_Mosaic_13Bands_nodata0.tif'


file_land_name=ImageInput.split("/")[-1].split(".")[0]
print(file_land_name)
...
if __name__ == '__main__':
    ...
    #else:
    N_Cls=1
    n_channels=4#12#8#13
    
    folder_model='./weights/'
    name_model="model_unet_20_32_4_1_percentile_min_max_HW"#'model_unet_150_32_13_1_min_max_River'
    ...
    save_fig_path=save_path+'/'+ name_model

    print("----------------------------------------------------")
    img=tiff.imread(ImageInput)
    ...
    img= np.delete(img,[0,4,5,6,8,9,10,11,12],2)
    ...
    cut_i=img.shape[0]//4
    cut_j=img.shape[1]//4
    img=img[cut_i:cut_i*3,cut_j:cut_j*3,:]    
    ...
    plt.savefig(save_fig_path+"_ww_"+ "{}_mask_ww_{}.png".format(file_land_name,trs_river),dpi=1200)



    ds = gdal.Open(ImageInput)
    driver = gdal.GetDriverByName("GTiff")
    cols = img_masked_river.shape[0]#ds.RasterXSize
    rows = img_masked_river.shape[1]#ds.RasterYSize
    outdata = driver.Create(save_fig_path+"_ww_"+ "{}_mask_ww_{}.tif".format(file_land_name,trs_river), rows, cols, 1, gdal.GDT_Byte)

    p0,p1,p2,p3,p4,p5=ds.GetGeoTransform()
    p0=p0+cut_j*p1
    p3=p3+cut_i*p5
    outdata.SetGeoTransform((p0,p1,p2,p3,p4,p5))##sets same geotransform as input
    outdata.SetProjection(ds.GetProjection())##sets same projection as input
    outdata.GetRasterBand(1).WriteArray(img_masked_river)
