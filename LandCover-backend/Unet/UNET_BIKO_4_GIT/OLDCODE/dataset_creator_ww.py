...
def dataset_creator(base_path):
    ...
    #train
    x_train_name = natsorted([f for f in glob(base_path+'TRAIN/ww/'+'*.tif')])
    y_train_name = natsorted([f for f in glob(base_path+'TRAIN/ww//'+'*.jpg') if 'Mask' in f])

    #test
    x_val_name = natsorted([f for f in glob(base_path+'TEST/ww/'+'*.tif')])
    y_val_name = natsorted([f for f in glob(base_path+'TEST/ww/'+'*.jpg' ) if 'Mask' in f])

    #rgb val
    x_val_RGB_file  = natsorted([f for f in tqdm(glob(base_path+'TEST/ww/'+'*.jpg')) if not 'Mask' in f])
    x_train_RGB_file  = natsorted([f for f in tqdm(glob(base_path+'TRAIN/ww/'+'*.jpg')) if not 'Mask' in f])
    ...
    np.save(save_path+'x_train_ww.npy',x_train_final)
    np.save(save_path+'y_train_ww.npy',y_train_final)
    np.save(save_path+'x_val_ww.npy',x_val_final)
    np.save(save_path+'y_val_ww.npy',y_val_final)

    np.save(save_path+'x_train_RGB_ww.npy',x_train_final_RGB)

    np.save(save_path+'x_val_RGB_ww.npy',x_val_final_RGB)
