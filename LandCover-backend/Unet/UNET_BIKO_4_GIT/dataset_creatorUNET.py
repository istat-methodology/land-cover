...
base_path='/IstatDL/Users/public/Datasets/UNET_DATASETS/Unet_ww/'
...
def dataset_creator(base_path):
    ...
    x_train_RGB_file  = natsorted([f for f in tqdm(glob(base_path+'TRAIN/'+'*.jpg')) if not 'Mask' in f])
...
#    x_val_aug=np.delete(x_val_aug,[9],1)
    ...
    x_train_aug=np.delete(x_train_aug,[0,4,5,6,8,9,10,11,12],1)
    x_val_aug=np.delete(x_val_aug,[0,4,5,6,8,9,10,11,12],1)
    ...
    np.save(save_path+'x_train.npy',x_train_final)
    np.save(save_path+'y_train.npy',y_train_final)
    np.save(save_path+'x_val.npy',x_val_final)
    np.save(save_path+'y_val.npy',y_val_final)

    np.save(save_path+'x_train_RGB.npy',x_train_final_RGB)

    np.save(save_path+'x_val_RGB.npy',x_val_final_RGB)
