...
def show_prediction(x,y_true,y_pred,idx_fig,label_list,trs=0.5):
    ...
    f,axx=plt.subplots(n_fig,5,figsize=(24,24))

    axx[0,0].set_title("RGB Image\n\n")
    axx[0,1].set_title('Val Mask River\n\n')
    axx[0,2].set_title('Val Mask Lake\n\n')
    axx[0,3].set_title('Val Pred River\n\n')
    ...
    for  idx in range(idx_fig[0],idx_fig[1]):
        ...
        axx[i,3].imshow(tmp_1,cmap=cmap_river_pred,vmin=0,vmax=1)
        axx[i,3].set_xlabel('jaccard_val: {}'.format(np.round(jac_1,3)))
